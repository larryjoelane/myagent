...
`.myagent/sessions/index.db`. The companion `recall-store` tool
(`/recall-save`) writes freeform facts into the same index.

## Invocation

```
node "$(git rev-parse --show-toplevel)/.claude/skills/recall/recall.js" "<query>" [--limit N] [--kind <kind>] [--stats] [--ingest]
```
...
explicit ask — answer the question. But for a bare `/recall
