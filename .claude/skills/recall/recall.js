...
//
// Re-execs the implementation (recall-impl.js, its sibling) under
...
if (RUNNING_UNDER_ELECTRON) {
  ...
  try {
    ...
    process.stderr.write(
      `recall: cannot find Electron — run \`npm install\` in the project root.\n` +
      `  underlying error: ${err.message}\n`,
    );
    process.exit(1);
  }
  const impl = path.join(__dirname, 'recall-impl.js');
  const child = child_process.spawnSync(electronBin, [impl, ...process.argv.slice(2)], {
    stdio: 'inherit',
    env: { ...process.env, ELECTRON_RUN_AS_NODE: '1' },
  });
  if (child.error) {
    process.stderr.write(`recall: failed to launch Electron — ${child.error.message}\n`);
    process.exit(1);
  }
