#!/usr/bin/env node
// recall implementation. Invoked by .claude/skills/recall/recall.js, which
...
function loadSessionIndex() {
  if (!_sessionIndex) _sessionIndex = require(path.join(PROJECT_ROOT, 'src', 'core', 'sessionIndex'));
...
function printHelp() {
  process.stderr.write([
    'recall — query the MyAgent session index',
    '',
    'Usage: node .claude/skills/recall/recall.js [options] <query>',
    '',
    'Options:',
  ...
  if (!args.forceLocal) {
    ...
    if (remote) {
      try {
        ...
        process.stderr.write(`recall: server error, falling back to local — ${err.message}\n`);
      }
    }
  }
