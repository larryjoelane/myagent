...
This is what `@memory <query>` and `bin/memory-search.js` do as of now,
...
- `bin/memory-search.js` accepts `--limit N` (default 10) and
...
- `bin/memory-search.js` is exercised directly in
