#!/usr/bin/env node
// CLI shim over src/core/sessionIndex.js. Same hybrid (FTS5 + cosine) search
// the Electron app exposes via IPC, but callable from any terminal — which
// is what `claude` running inside a PTY pane needs, since it can't reach
// the Electron main process directly.
//
// Usage:
//   node bin/memory-search.js "auth middleware refactor"
//   node bin/memory-search.js --limit 5 --kind agent-in "what did we say about caching"
//   node bin/memory-search.js --ingest               # force re-ingest, no search
//   node bin/memory-search.js --stats                # row/vector/file counts
//
